class PageRenderer {
  ...
  renderIndexPage({ posts, pagePosts, pageNumber, totalPages }) {
    ...
    const hero = pageNumber === 1 ? this.renderHomeHero(blogPosts.length, posts.length) : this.renderArchiveHero(pageNumber);
  ...
  renderHomeHero(blogCount, totalCount) {
    return `
      <section class="home-hero reveal-on-scroll">
        <div class="hero-copy">
          ...
          <p class="post-meta">Includes ${blogCount} blog${blogCount === 1 ? "" : "s"} across ${totalCount} total entries.</p>
        </div>
      </section>
    `;
  }
