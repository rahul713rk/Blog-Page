...
class PageRenderer {
  ...
  wrapPage({ title, description, content, canonicalPath = "", head = "" }) {
    return this.templates.render("layout.html", {
      ...
      head,
      blogsActive:
        canonicalPath === "posts" || canonicalPath.startsWith("posts/") ? 'aria-current="page"' : "",
      guidesActive:
        canonicalPath === "guides" || canonicalPath.startsWith("guides/") ? 'aria-current="page"' : "",
      aboutActive:
        canonicalPath === "about" || canonicalPath.endsWith("/about/index.html")
          ? 'aria-current="page"'
          : "",
      homeActive: canonicalPath === "" ? 'aria-current="page"' : ""
    });
  }
  ...
  renderHomePage(posts) {
    const hero = this.renderHomeHero(posts.filter(p => p.kind === 'blog').length, this.context.config);
    const about = this.renderAboutSection();
    const search = this.renderSearchSection();
    const tagDirectory = this.renderTagDirectory(posts);
    ...
    return this.wrapPage({
      title: this.context.config.siteName,
  ...
  renderArchivePage({ posts, pagePosts, pageNumber, totalPages }) {
    const postsHtml = pagePosts.map((post) => this.renderPostCard(post)).join("\n");
    const pagination = this.renderPagination("posts", pageNumber, totalPages);
    const hero = pageNumber === 1 ? this.renderArchiveHero(1, "Latest Posts") : this.renderArchiveHero(pageNumber);
    const library = this.renderArchiveLibrary(postsHtml, pagination);
    ...
    return this.wrapPage({
      title:
        pageNumber === 1
          ? `Blogs | ${this.context.config.siteName}`
          : `Blogs - Page ${pageNumber} | ${this.context.config.siteName}`,
      description: `Technical blogs and articles by ${this.context.config.ownerName || "the author"}.`,
      content,
      canonicalPath: pageNumber === 1 ? "posts" : `posts/page/${pageNumber}`
    });
  }

  renderGuidesPage(posts) {
    const tutorialTracksHtml = this.renderTutorialTracks(posts);
    const hero = `
      <section class="archive-hero reveal-on-scroll">
        <h1>Practical Guides</h1>
        <p>Deep-dive learning paths and structured technical notes.</p>
      </section>
    `;
    const library = `<div class="guides-archive reveal-on-scroll">${tutorialTracksHtml}</div>`;
    ...
    return this.wrapPage({
      title: `Guides | ${this.context.config.siteName}`,
      description: "Step-by-step technical guides and learning modules.",
      content,
  ...
  renderPagination(baseUrl, pageNumber, totalPages) {
    ...
    const prevHref = this.context.withSitePath(baseUrl + "/page/" + (pageNumber - 1));
    const nextHref = this.context.withSitePath(baseUrl + "/page/" + (pageNumber + 1));

    const prev =
      pageNumber > 1
        ? `<a href="${prevHref}" class="pagination-nav" rel="prev">Prev</a>`
        : `<span class="pagination-nav is-disabled">Prev</span>`;
    const next =
      pageNumber < totalPages
        ? `<a href="${nextHref}" class="pagination-nav" rel="next">Next</a>`
        : `<span class="pagination-nav is-disabled">Next</span>`;

    return `<nav class="pagination">
      ${prev}
      <span class="pagination-select"><span>Page ${pageNumber} of ${totalPages}</span></span>
      ${next}
    </nav>`;
  }

  renderHomeHero(blogCount, config) {
    return renderHomeHero(blogCount, config);
  }
  ...
  renderTagDirectory(posts) {
    ...
    const tagHtml = sortedTags
      .map(([tag, count]) => {
        const tagSlug = this.textUtils.slugify(tag);
        return `<a href="${this.context.tagHref(tagSlug)}" class="tag-summary">${this.textUtils.escapeHtml(
          tag
        )} <small>${count}</small></a>`;
      })
      .join("\n");
    ...
    return `
      <section class="tag-directory reveal-on-scroll">
        ...
        <div id="tag-directory-panel" class="section-collapse" data-collapsible-panel="tag-directory">
          <div class="section-collapse-inner">
            <div class="tag-directory">${tagHtml}</div>
          </div>
        </div>
      </section>
    `;
    ...
    return `
      <section class="about-section reveal-on-scroll" id="about-section">
        ...
        <div class="about-card">
          <p class="about-copy">${this.textUtils.escapeHtml(this.context.config.aboutBlurb || "Software engineer passionate about systems architecture...")}</p>
        </div>
      </section>
    `;
  }
    ...
    return `
      <div class="home-library">
        <section class="posts-library reveal-on-scroll">
          <header class="section-heading"><h2>Latest Posts</h2></header>
