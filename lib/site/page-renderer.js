class PageRenderer {
  ...
  renderTagPage({ tagName, pagePosts, pageNumber, totalPages, tagSlug }) {
    ...
    return this.wrapPage({
      ...
      head: `<script type="module" src="${this.context.withSitePath("assets/js/site.js")}"></script>`
    });
  }
  ...
  renderHomeHero(posts, blogCount) {
    ...
    const tutorialPosts = totalCount - blogCount;
    const guideCollections = new Set(
      posts.filter((post) => post.kind === "tutorial").map((post) => post.collectionSlug)
    );
    const topicCount = this.collectTagSummaries(posts).length;
    const totalMinutes = posts.reduce((sum, post) => {
      const minutes = Number.parseInt(String(post.readingTime || ""), 10);
    ...
    return `
      <section class="home-hero reveal-on-scroll">
        <div class="hero-copy">
          ...
          <p class="post-meta">Curated notes, practical blogs, and structured guide tracks for backend, DevOps, and system design.</p>
        </div>
        <div class="hero-stats" aria-label="Site statistics">
          <article class="hero-stat-card">
            <span class="stat-label">Published</span>
            <strong>${totalCount}</strong>
            <small>Total entries in the library</small>
          </article>
          <article class="hero-stat-card">
            <span class="stat-label">Blogs</span>
            <strong>${blogCount}</strong>
            <small>Standalone articles and writeups</small>
          </article>
          <article class="hero-stat-card">
            <span class="stat-label">Guide Tracks</span>
            <strong>${guideCollections.size}</strong>
            <small>${tutorialPosts} lessons organized into collections</small>
          </article>
          <article class="hero-stat-card">
            <span class="stat-label">Depth</span>
            <strong>${topicCount}</strong>
            <small>${totalMinutes} minutes of indexed reading</small>
          </article>
        </div>
      </section>
    `;
  ...
  renderTagDirectory(posts) {
    ...
    return `
      <section class="content-section reveal-on-scroll">
        ...
        <div class="section-collapse is-open" id="tag-directory-panel" data-collapsible-panel="tag-directory">
