...
class PageRenderer {
  ...
  renderIndexPage({ posts, pagePosts, pageNumber, totalPages, tutorialTracksHtml }) {
    ...
    const hero = pageNumber === 1 ? this.renderHomeHero(blogPosts.length, this.context.config) : this.renderArchiveHero(pageNumber);
  ...
  renderTagDirectory(posts) {
    ...
    return `
      <section class="tag-directory reveal-on-scroll">
        ...
        <div id="tag-directory-panel" class="tag-directory-panel is-open" data-collapsible-panel="tag-directory">
