...
function renderPostPager(post, { textUtils }) {
  const prev = post.previousPost
    ? `<a href="${post.previousPost.url}" class="page-jump prev"><span>&larr; Prev</span><strong>${textUtils.escapeHtml(
        post.previousPost.title
      )}</strong></a>`
    : "<div></div>";
  const next = post.nextPost
    ? `<a href="${post.nextPost.url}" class="page-jump next"><span>Next &rarr;</span><strong>${textUtils.escapeHtml(
        post.nextPost.title
      )}</strong></a>`
    : "<div></div>";

  return `<nav class="page-jumps">${prev}${next}</nav>`;
}
