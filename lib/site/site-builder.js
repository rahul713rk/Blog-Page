  ...
  build() {
    ...
    try {
      if (require("fs").existsSync(cachePath)) {
        cache = JSON.parse(require("fs").readFileSync(cachePath, "utf8"));
      }
    } catch { }
    ...
    posts.forEach((post) => {
      ...
      if (cache[post.sourcePath] === mtime && require("fs").existsSync(path.join(this.context.publicDir, post.outputPath))) {
        skippedCount++;
        return;
      }
    ...
    require("fs").writeFileSync(cachePath, JSON.stringify(newCache, null, 2));

    console.log(`Built site. Rendered ${posts.length - skippedCount} posts, skipped ${skippedCount}.`);
  }
...
<rss version="2.0">
  ...
  generateSitemap(posts) {
    ...
    const indexPages = Array.from({ length: totalPages }, (_, i) => 
      i === 0 ? "" : `page/${i + 1}`
    );
