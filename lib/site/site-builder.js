...
<rss version="2.0">
  ...
  generateIndexPages(posts) {
    // 1. Generate Home Page (Landing)
    this.writeRelative("index.html", this.pageRenderer.renderHomePage(posts));

    // 2. Generate Blog Archive Pages (under /posts/)
    const blogPosts = posts.filter((post) => post.kind === "blog");
    const paginatedPosts = this.textUtils.paginate(blogPosts, this.context.config.postsPerPage || 5);
    ...
    paginatedPosts.forEach((pagePosts, index) => {
      ...
      const outputPath = pageNumber === 1 ? "posts/index.html" : `posts/page/${pageNumber}/index.html`;

      this.writeRelative(
        outputPath,
        this.pageRenderer.renderArchivePage({
          posts,
          pagePosts,
          pageNumber,
    ...
    // 3. Generate Guides Index Page (under /guides/)
    this.writeRelative("guides/index.html", this.pageRenderer.renderGuidesPage(posts));
  }
