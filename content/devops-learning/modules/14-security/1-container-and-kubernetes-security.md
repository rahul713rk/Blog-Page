...
kubectl auth can-i create pods --as system:serviceaccount:app:default
~~~
...
## Diagram
~~~text
Plan/Design -> Threat Review
      |
      v
Code/PR -> SAST + Secret Scan
      |
      v
Build -> SCA + Image Scan
      |
      v
Staging -> DAST + Policy Tests
      |
      v
Production -> Runtime Detection + Audit
~~~

## Real World Use Case
Teams in regulated environments embed DevSecOps controls into pull requests and release pipelines so risky changes are blocked early. They combine scanner results with ownership and remediation SLAs, which improves security posture while preserving delivery speed.

In mature organizations, this topic is rarely isolated. It connects to release governance, incident response, cost control, and platform standards. Teams that operationalize Container and Kubernetes Security usually define scorecards (lead time, failure rate, rollback rate, MTTR, policy compliance) and review trends every sprint. The review process is where tactical work becomes strategic improvement.
...
## Exercises
1. Write a one-page implementation brief for Container and Kubernetes Security including objective, owners, constraints, and success metrics.
...
6. Run a tabletop incident scenario related to Container and Kubernetes Security; document timeline, decisions, and improvement actions.
